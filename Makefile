...
##
## Cross-Platform Makefile generated from 'NoteSorter.mk' project file.
##
...
##	headers - Installs the headers into the parent project's include directory.
...
LIBOUTPUTDIR = $(OUTPUTDIR)/$(LIBDIR)/$(SYSTEM)/
...
shared: init $(OBJECTS)
	...
	@echo $(MAKECMDGOALS) | grep debug; \
	if [ $$? -eq 1 ]; then \
		cp lib$(NAME)$(LIBEXT) $(ROOT)/$(LIBDIR)/$(SYSTEM)/; \
	fi
...
HEADERS := $(shell cd $(INCLUDEDIR) && find * -type f)
headers: init $(HEADERS)
	mkdir -p $(ROOT)/$(INCLUDEDIR)/$(PARENT)/$(NAME)
	cp -a $(INCLUDEDIR)/* $(ROOT)/$(INCLUDEDIR)/$(PARENT)/$(NAME)
...
.PHONY: all debug release test shared static headers dist package install \
		uninstall init clean distclean docs project
